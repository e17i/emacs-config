...
In emacs lisp, we want slurp and barf support via smartparens. to be continued..
...
#+BEGIN_SRC emacs-lisp
(define-key elpy-mode-map (kbd "<backtab>") 'elpy-folding-toggle-at-point)
#+END_SRC

Also use flycheck in python.
...
current line of point (not only for python).
...
*** TODO think about movement/search placing point on beginning of adjacent object
