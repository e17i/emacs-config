...
#+BEGIN_SRC emacs-lisp
(global-set-key "\C-x^" 'resize-window)
#+END_SRC
...
(defun activate-german-keymap ()
  ...
  (global-set-key "\242" 'mark-word) ; M-@
  (global-set-key "\246" 'delete-indentation) ; M-^
  (global-set-key "\C-x&" 'resize-window) ; C-x ^ (what about rebind?)
...
~suspend-frame~. Additionally, always enter evil mode in emacs state. This
