...
The last step now still missing: when yanking into an active region, while the
region is replaced by the last (or previous) contents of the kill ring, the
replaced contents of region, in this case, is not pushed to the kill ring.
Which would make it very convenient (and conforming to expectations) to just
use =C-(y y)=, first replacing the region with the head of the kill ring, then
inserting the new head (the text just replaced). Of course, you can achieve
exactly this by deactivating the region and then inserting the yank. But
still, when assigning a register to ~delete-selection-save-to-register~, the
text replaced by a yank is saved to that register. Now just re-insert it with
=s-r i y= (for instance, if you assigned the =y= register). And again,
