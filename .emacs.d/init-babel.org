...
first view seems to be not such a bad selection. 

But we also need to make sure windows are not splitted if too small.

#+BEGIN_SRC emacs-lisp
(custom-set-variables
 '(display-buffer-base-action
   '((display-buffer--maybe-same-window display-buffer-reuse-window display-buffer--maybe-pop-up-frame-or-window display-buffer-in-previous-window display-buffer-below-selected display-buffer-pop-up-frame)))
 '(window-min-height 8)
 '(window-min-width 40)
 )
#+END_SRC
...
(defun display-buffer-reuse-help (buffer alist)
  ...
  (let ((wnd (car (remove-if-not
    ...
    (if (and wnd (window-live-p wnd))
        (progn
          (message (format "resize..%s " alist))
          (window--display-buffer buffer wnd 'window alist)))))
;; FIXME make a function respecting window size args and recording with reuse..
#+END_SRC
...
We also put in our ~dired~ functions which see.
...
(custom-set-variables
 '(display-buffer-alist
   ((dired-to-other-window nil)
    (dired-from-other-window
     (display-buffer-in-orig))
    ("\\*Help\\*\\|\\*Apropos\\*"
     (display-buffer-dedicated-window display-buffer-reuse-help display-buffer-at-bottom)
  ...
  (if (cdr (assq 'inhibit-same-window alist))
      (let ((my-window
        ...
        (if (and my-window (window-live-p my-window))
            (window--display-buffer buffer my-window 'window alist)))))
#+END_SRC
...
This done, we have no more window reusing, so we can leave or remove windows at
will. A last resort is still the push to frame key defined below.

**** Dedicated window

These functions are not yet matured. Use them to define your dedicated 'other'
window, but probably only with a selection regex or function limiting its
application to defined buffer types. The setting of
~other-window-scroll-buffer~ is handled very generically with the aim of
making the other window key definitions below more functional. So probably use
this only with care and for selected buffer regexps or selector functions.
...
Always set the other window scroll buffer to enable my remote buffer accessing
functions. So enable this only with specified selectors."
  (setq other-window-scroll-buffer buffer)
  (if (and dedicated-other-window
           (window-live-p dedicated-other-window))
      (window--display-buffer buffer dedicated-other-window 'window alist)))
#+END_SRC
...
**** TODO continue experiments with-current-buffer
...
*** TODO find out about alist handling
