...
(defun eshell-copy-region (&optional arg)
  ...
  (interactive "P")
  (let ((input (eshell-get-old-input arg)))
    (deactivate-mark)
    ...
    (if (null overlays)
      ...
      (let ((invs (caar overlays))
        ...
        (let ((start (if (> invs beg) beg
                       (smaller (larger inve beg) end)))
              (stop (if (< inve end) end
                      (larger (smaller invs end) beg))))
          (let ((inv (smaller invs stop))
                (next (larger inve start)))
