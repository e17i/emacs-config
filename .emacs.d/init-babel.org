...
(custom-set-variables
 ...
 '(display-buffer-alist (("\\*Help\\*" display-buffer-dedicated-window
                          (window-height . fit-window-to-buffer-max-half-frame)))))
#+END_SRC

We augment it with a buffer specific ~window-height~ action attribute only for
help buffers, specifying our custom function
...
~other-window-scroll-buffer~ should happen at a more generic place, making the
other window key definitions bekow more functional.
...
selected. If none is selected, revert to the default behaviour. Also,
if special action attributes are defined, use the default."
  (if (and (not alist)
