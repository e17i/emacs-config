#+TITLE: Jˆrgs Emacs init
* Emacs Configuration

This configuration consists of three parts. In the first one, I'm collecting
lisp functios. Some of them maybe should be put into proper packages
instead. The second part defines global key bindings. I see it as main
part, as the overall emacs behaviour is defined there.
In the third part, I'm placing configurations for the packages I use.
...
I like to have an easy way setting and changing colors and
...
First of all, the configuration, including changes to
...
also attributes modifying the window size.I want to put those buffers into windows at bottom,
where they are unobtrusive and can stay as long as we want.
If such a window already exists, I want to reuse it. If I previously
defined a special dedicated window (see below) I want that to be used.
The given ~display-height~ attribute makes sure the window (also if
reused) is shrunk to it's actual contents. If it needs to be widened
instead, this is done only to half of the frame size.
This way, I can describe functions or variables as often as I need and
the description always goes into the same window at the bottom,
...
Now I'm defining a buffer specific ~window-height~ action attribute only for
help and apropos buffers, specifying my custom function
~fit-window-to-buffer-max-half-frame~, which probably may need more tweaking to use less
...
The sasme should work in a similar way for ~dired~ buffers. But here
is more too do besides selecting a window for the dired buffer. If used in
the same window (e.g. via =C-x C-j=) , nothing to be done. But when
used in the other window (=C-x 4 C-j=), it should memorize its origin window and put
view buffers sent to /subsequent/ other windows there. So I can open
dired in the other window (or leave it there and later on jump back to
it), select other files and open them (=o=) within my main window,
this way jumping forth and back between main and dired window.
...
display the dired buffer."
  (let ((my-window (get-window-with-predicate
                    (lambda (w)
                      (eq :dired (window-parameter w 'orig-window))))))
    (if (and my-window (window-live-p my-window))
        (window-update-buffer buffer my-window 'reuse alist))))
...
First, some functions recording a buffer to be shown in another
window. It should be remote controllable as long as it is visible.
When removed and no new buffer pushed to another window, we will fall
back to the remote controlling behaviour selecting the next window.
...
..and a function coding the logic to find the other window. The
...
Probably the idea of having =M-x= also on a /Control/ binding proves to be good.
...
(global-set-key "\C-x40" #'kill-buffer-and-window)
#+END_SRC
...
Example of defining an own vi mode. Motivated by the convenient navigation
