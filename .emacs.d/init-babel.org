...
(defun deadgrep-display-result ()
  "display currently selected match, but don't visit/select."
  (interactive)
  (let ((current-window (selected-window)))
    (deadgrep-visit-result)
    (select-window current-window)))

(defun deadgrep-display-next ()
...
(define-key deadgrep-mode-map "\C-o" #'deadgrep-display-result)
#+END_SRC
...
are different ways to archieve this. First of all, with =C-x C-f= you
can activate ~error-follow-minor-mode~, allowing to move the mark
...
of addressing the other window (=C-x 4=) and starting the goto
sequence (=M-g=) needs to be casted only as an initiation. Afterwards,
...
using =multi-occur= to find a common regex within a project's files,
arrange for a current file to be displayed within a (selected) main window and
multi-occur in another window. Now just call =next-error-other-window=
...
(defun next-error-other-window (&optional arg)
  "Visit next error message displayed in other window's buffer. With prefix arg, jump to nth window."
  (interactive "P")
  (with-current-buffer (window-buffer (call-interactively #'find-other-window))
    (next-error arg)
    (error-step)))

(defun previous-error-other-window (&optional arg)
  "Visit previous error message displayed in other window's buffer. With prefix arg, jump to nth window."
  (interactive "P")
  (with-current-buffer (window-buffer (call-interactively #'find-other-window))
    (previous-error arg)
    (error-step)))
...
On X11, set a convenient frame size.

#+BEGIN_SRC emacs-lisp
(defun init-frame-on-x11 ()
  "Set frame size to a prescribed convenient size."
  (interactive)
  (set-frame-width nil 100)
  (set-frame-height nil 40))

(defun fake-small-fullsize-on-x11 ()
  "Set frame size to a prescribed fullsize on mobile display."
  (interactive)
  (set-frame-position nil 10 320)
  (set-frame-width nil 160)
  (set-frame-height nil 45))

(defun fake-med-fullsize-on-x11 ()
  "Set frame size to a prescribed larger size."
  (interactive)
  (set-frame-width nil 186)
  (set-frame-height nil 55))

(defun fake-large-fullsize-on-x11 ()
  "Set frame size to a prescribed fullsize on stationary display."
  (interactive)
  (set-frame-position nil 1460 30)
  (set-frame-width nil 236)
  (set-frame-height nil 65))

(define-prefix-command 'fake-frame-size)
(global-set-key "\C-ci" #'fake-frame-size)
(define-key fake-frame-size "0" #'init-frame-on-x11)
(define-key fake-frame-size "1" #'fake-small-fullsize-on-x11)
(define-key fake-frame-size "2" #'fake-med-fullsize-on-x11)
(define-key fake-frame-size "9" #'fake-large-fullsize-on-x11)
#+END_SRC
...
(defun frame-to-second-x11 (frame lines cols)
  ...
  (set-frame-width frame 159)
  (set-frame-height frame 57))
#+END_SRC
...
expand-region may come handy, besides the explicit selection commands.
