  ...
  (lexical-let ((arg-acc 0)
    ...
    (labels ((get-arg-acc (&optional default)
	     ...
	     (arg-negate ()
			 ...
			 (message " %c %d" (if arg-neg arg-neg '? ) arg-acc))
	     (arg-accumulate (num)
      ...
      (macrolet ((with-window (&rest body)
				 ...
				 (with-selected-window window
				   ,@body))))
	(message "navigate ([-,0-9] n,p,N,P,v,V,<,>,[,],l, )")
	(set-transient-map
	 (let ((map (make-sparse-keymap)))
...
Mouse keys: remember basic mouse button handling in emacs: left - set
pointer, double left - select word, drag - select, drag after double -
select words, right - define region and save to kill ring, right after
double click - define region up to next word, double right - kill to
kill ring, middle - yank to click position.
...
#+BEGIN_SRC emacs-lisp
  ;; mac trackpad: select: mouse-1, copy/cut: mouse-3, yank: M-mouse-2
...
;; (setq c-basic-offset 2) ;; already in c++-mode hook?
(setq minibuffer-max-depth nil)  ;; what's this?

;; (server-start) ;; replace with custom server-mode
...
#+BEGIN_SRC emacs-lisp
;; (global-set-key "\C-cc" 'compile)  ;; here probably use make better.. 
...
;; (put 'eval-expression 'disabled nil)  ;; needed?
#+END_SRC
