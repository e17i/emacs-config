...
In enter functions, you may insert direct keys into my-environment-mode-map or
C-c prefixed keys into my-env-map. "
...
You may add key definitions to the ~my-environment-mode-map~ in enter
functions, or =C-c= prefixed keys into ~my-env-map~.
To remove them on exit, use =my-env-reset-keymap=.
...
function should call this function to reset it."
  (setq my-environment-mode-map (my-env-init-keymap)))
#+END_SRC
...
(defun activate-german-keymap ()
  ...
  (define-key my-environment-mode-map "\242" 'mark-word) ; M-@, here M-"
  (define-key my-environment-mode-map "\246" 'delete-indentation) ; M-^, here M-&
  (define-key my-environment-mode-map "\C-x&" 'enlarge-window) ; C-x ^, here C-x &
  (define-key my-environment-mode-map "\C-x*" 'enlarge-window-horizontally) ; C-x }
  (define-key my-environment-mode-map "\C-xÜ" 'shrink-window-horizontally)) ; C-x {
#+END_SRC
...
How emacs finds it's tools:

#+BEGIN_SRC emacs-lisp
(setenv "PATH" (concat "/opt/local/bin:/opt/local/sbin:" (getenv "PATH")))
#+END_SRC

This may evolve into real keyboard handling..
...
*** TODO env-mode: use make-composed-keymap, set-keymap-parent
