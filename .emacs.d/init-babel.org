...
(defun display-buffer-in-orig (buffer alist)
  ...
  (let* ((other-buffer (current-buffer))
         (my-window (find-orig-window other-buffer)))
...
So when a ~dired-to-other-window~ matches, we need a function marking the
...
And vice versa, when a ~dired-from-other-window~ matches (this should actually
be called something like ~from-dired-to-orig-window~), the new buffer should
