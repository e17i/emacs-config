# -*- org-html-postamble-format:(("en" "<p class="author">Author: %a</p> <p>Made on emacs org-mode</p>"));
...
#+INFOJS_OPT: view:overview toc:t path:chrome/org-info.js
#+LATEX_CLASS: article
#+LATEX_CLASS_OPTIONS: [landscape]
#+LATEX_HEADER: \usepackage[a4paper]{anysize}
#+AUTHOR: Jˆrg Kollmann
#+TITLE: My Emacs init
* Emacs Configuration
...
In the third part, package configurations for all packages I use are collected.

I'm not (yet) using =use-package=. I want to distribute my config over
...
To bootstrap a new emacs, after importing this file the packages required here
must be installed. To achieve this, just start =emacs --no-init=, set the
=package-archives= and =package-selected-packages= variables to the lists given
in custom.el, and call =M-x package-install-selected-packages=.
...
~display-buffer-base-action~ or in ~display-buffer-alist~. Here we just replace the
~display-buffer-use-some-window~ function which probably is the source of
those erratic behaviours. Our replacement is a combination of
...
=C-M-c= leaves a level of recursion.

#+BEGIN_SRC emacs-lisp
(defun edit-with-fallback-display ()
  "reset `display-buffer-alist' and edit recursively, to
...
And, similarly, use this to make sure the next other windows will be opened
below all other windows.

#+BEGIN_SRC emacs-lisp
(defun edit-with-display-at-bottom ()
  "set `display-buffer-alist' and edit recursively, to
  push the next buffer to the screen's bottom."
  (interactive)
  (let ((display-buffer-alist '((display-buffer-at-bottom))))
    (recursive-edit)))
#+END_SRC

*** debugging display buffers
...
~(walk-windows (lambda (w)
                (print (format "window %s orig %s" w (window-parameter w 'orig-window)))))~

Use ~(set-orig-window (selected-window) :dired)~ to explicitely set the
current window as dired origin.

Check for current dired origin with this: ~(find-orig-window :dired)~.
Remove dired marker with this: ~(clear-orig-window-marker :dired)~.

When having set the orig window as :dired window, you subsequently want to
open further buffers in other window using the orig-window system (e.g. occur
or deadgrep). To make this work, each window can get different orig-window
markers without overwriting each other.

This corresponds to "the emacs way" of editing, so you don't have just one
editing area but instead can start from each window, opening occur/grep/dired
others to select and open files in the corresponding originating window,
whether it is the same for all these operations or not.

When designating an origin window, a target window is not yet known. Insofar
the target buffer is used. So the designated window is then origin for the
given buffer. This might work for occur and deadgrep, but for dired it is
common to navigate and this way to change the current buffer in the dired
window. Thats why instead a :dired marker has been used. So each dired buffer
visible in any window will open new files in the same origin window.

*** Using side windows

You can push existing buffers into side windows, so they wont be replaced by
other buffers when pushing buffers into other windows in the main working
area. Also the visibility of side windows can be triggered by =C-x w s=
respective =s-w s=.

As triggering the visibility possibly constructs new windows, the window
parameters and origin designations get lost.

#+begin_src emacs-lisp
(defun push-buffer-to-side (buffer prefix)
  "Push buffer into a new side window specified by prefix arg, where 0 is left,
1 is right, 2 is top and 3 is bottom."
  (interactive "bbuffer: \ncdirection (l, r, t, b): ")
  (let ((where (cond
                ((equal prefix ?l) 'left)
                ((equal prefix ?r) 'right)
                ((equal prefix ?t) 'top)
                ((equal prefix ?b) 'bottom)))
        (display-buffer-alist nil))
    (display-buffer buffer
                    `(display-buffer-in-side-window . ((side . ,where))))))
#+end_src

And, using a similar approach to set up a common working environment, we could
use something like this:
#+begin_src emacs-lisp
(defun my-ide ()
  "When called with a source file inside of some project
 directory opened, push corresponding dired, occur and ansi-term
 buffers into side windows."
  (interactive)
  (let ((display-buffer-alist
         '((new-buffer-is-dired-p
            (make-dired-window-origin display-buffer-in-side-window)
            (side . left))
           ("\\*Occur\\*"
            (make-buffer-window-origin display-buffer-in-side-window)
            (side . right))
           ("\\*ansi"
            display-buffer-in-side-window
            (side . bottom))))
        (term (car (cl-remove-if-not (lambda (name) (string-match "\\*ansi" name)) (mapcar #'buffer-name (buffer-list)))))
        (w (selected-window))
        (b (current-buffer)))
...
recognized by a regex, receive a special list of display actions and
...
See above for debugging aids when working with window markers.
...
window can be found containing this marker."
  (walk-windows (lambda (w)
                  (set-window-parameter w 'orig-window
                                        (remove marker-or-buffer (window-parameter w 'orig-window))))))
...
subsequent other window view buffers into the origin window."
  (unless (eq (window-buffer window) marker-or-buffer)
    (clear-orig-window-marker marker-or-buffer)
    (push marker-or-buffer (window-parameter window 'orig-window))))
#+END_SRC
...
(nth-window scroll-down-line-other-window
  "scroll down in other window."
  (scroll-down-line))

(nth-window scroll-up-line-other-window
  "scroll up in other window."
  (scroll-up-line))
...
This is now also implemented in the default window functions (=s-w ^ f=).
...
allowing to specify a remote window to be controlled."
  (let ((arg-acc 0) (arg-neg nil) (consumed nil)
	     ...
	     (get-arg-acc (&optional default)
			  (let ((res (if (> arg-acc 0)
					 (* arg-acc (if arg-neg -1 1))
				       default)))
			    res))
	     (acc-status () (message "navigate ([-,0-9] n,p,/, ,v,V,<,>,[,],l,r,q) %c %d"
...
When scrolling page wise (=C-v= and =C-V=), these functions help to control how much of the
previous page to stay visible. ~scroll-1~ leaves visible 90% of the previous
screen, doing only small scroll steps. ~scroll-2~ still leaves 70%.
~scroll-3~ jumps roughly halve of the screen size. Each of them enable a recursive
edit, which can be cancelled with =C-M-c=.
The default would be like ~scroll-4~, jumping for almost full pages.

Probably to be unified into a single function with argument..

#+BEGIN_SRC emacs-lisp
(defun scroll-2 ()
  "When scrolling pagewise, set the context lines to roughly 70% of screen height. This way, you need five steps to scroll one page."
  (interactive)
  (let ((next-screen-context-lines (round (* (window-total-height) 0.7))))
    (recursive-edit)))
...
(defun scroll-1 ()
  ...
  (interactive)
  (let ((next-screen-context-lines (round (* (window-total-height) 0.9))))
    (recursive-edit)))
...
(defun scroll-3 ()
  ...
  (interactive)
  (let ((next-screen-context-lines (round (* (window-total-height) 0.5))))
    (recursive-edit)))
#+END_SRC
...
And another temporary pseudo mode, here for interactively resizing windows.
  ...
  (macrolet ((msg ()
    ...
    (set-transient-map
     (let ((tmap (make-sparse-keymap)))
       ...
       (define-key tmap "‹" (lmb (shrink-window-horizontally 1))) ; german keyboard, here "U
...
receive their current FRAME as argument.
...
(defun switch-frame ()
  ...
  (interactive)
  (let ((maximized (frame-parameter nil 'my-fullscreen))
    ...
    (cond
     ...
     ((eq maximized 'maximized)
      (set-frame-parameter nil 'my-fullscreen nil)
      (set-frame-parameter nil 'my-confectioned 't)
      (dolist (fnc my-frame-stylings)
        (funcall fnc (selected-frame))))
     ((eq maximized nil)
      (set-frame-parameter nil 'my-geometry
                           (list (frame-position)
                                 (frame-width)
                                 (frame-height)))
      ;; on xquartz overwrite maximized with workarea values
      ;; (set-frame-parameter nil 'fullscreen 'maximized)
      (set-frame-parameter nil 'my-fullscreen 'maximized)
      (let ((geom (cdr (frame-geom nil))))
        (set-frame-position nil (car geom) (cadr geom))
        (set-frame-size nil (caddr geom) (- (cadddr geom) 20) t))))))

(defun frame-width-below-threshold (frame)
...
(defun frame-width-half (frame)
  "Set frame width half of display width."
  (set-frame-width frame (/ (frame-parameter frame 'width) 2)))

(defun frame-width-half-plus (frame)
  "Set frame width to some more than half of display width."
  (set-frame-width frame
                   (+ (/ (frame-parameter frame 'width) 2) 12)))

(defun frame-width-dec (frame)
  "Set frame width a little bit smaller than display width."
  (set-frame-width frame (- (frame-parameter frame 'width) 1)))

(defun frame-width-abs (frame)
...
(defun frame-height-dec(frame)
  "Set frame height a little bit smaller than display height."
  (set-frame-height frame (- (frame-parameter frame 'height) 1)))

(defun frame-height-dec-two(frame)
  "Set frame height a little bit smaller than display height."
  (set-frame-height frame (- (frame-parameter frame 'height) 2)))

(defun frame-height-dec-three(frame)
  "Set frame height a little bit smaller than display height."
  (set-frame-height frame (- (frame-parameter frame 'height) 3)))
...
(defun my-env-remove-map (kmap)
  ...
  (setq my-env-maps (remove-if (lambda (elem)
                                 (eq elem kmap))
                               my-env-maps)))
...
;; (global-set-key "\C-ch" 'er/expand-region) ::free this key for hide-lines
...
(global-set-key (kbd "C-M-s-p") #'scroll-down-line-other-window)
(global-set-key (kbd "M-s-p") #'scroll-down-line)
(global-set-key "\M-N" #'scroll-up-line)
(global-set-key (kbd "M-s-n") #'scroll-up-line)
(global-set-key (kbd "C-M-s-n") #'scroll-up-line-other-window)
(global-set-key "\M-F" #'forward-to-word)
(global-set-key (kbd "M-s-f") #'forward-to-word)
(global-set-key "\M-B" #'backward-to-word)
(global-set-key (kbd "M-s-b") #'backward-to-word)
...
(defun my-wheel-zoom ()
  (let ((kmap (make-sparse-keymap)))
    (define-key kmap (kbd "<S-mouse-7>") #'text-scale-increase)
    (define-key kmap (kbd "<S-mouse-6>") #'text-scale-decrease)
    (my-env-add-map kmap)))
...
(defun activate-german-keymap ()
  ...
  (let ((kmap (make-sparse-keymap)))
    ...
    (define-key kmap "\C-x‹" #'shrink-window-horizontally)  ; C-x {
...
Now, on a Linux notebook keyboard, again we want /Meta/ to be positioned left
of space and /Super/ to be left of that.
As a side note, in gnome different /Super/ shortcuts are defined; you may want
to undefine them. The key for =s-p= is not found in settings so use =gsettings
set org.gnome.mutter.keybindings switch-monitor "[]"= to unset it.

#+BEGIN_SRC emacs-lisp
(defun on-notebook-keyboard ()
  (custom-set-variables
   '(x-meta-keysym 'super)
   '(x-super-keysym 'meta)))
#+END_SRC

*** experimental handling of super modifier

The idea is to use /super/ instead of =C-x=. Especially for combinations
whose second key doesn't want a /Control/ modifier, this might be a saver of
keystrokes. So instead of pressing =C-x o=, you just hold /super/ and
repeatedly press =o= to iterate through windows. For this application, we also
bind =s--= to ~negative-argument~, allowing us to easily go backwards through
the window list.

Also, combinations that were not practical before can be used now. For
example, a combination like =C-x M-g=, which involves changing the modifiers,
violates the usage patterns. But, when using /super/ instead, it translates to
=M-s-g= which is completely viable. This way, a complete set of combinations
with modifier sets =M-s= and =C-M-s=, which are almost unused so far, is now
easily reachable.

#+begin_src emacs-lisp


(defun use-super-as-ctl-x ()
  (let ((kmap (make-sparse-keymap)))
    (define-key kmap (kbd "s-SPC") #'rectangle-mark-mode)  ; on mac this is spotlight, see next for an alternative
    (define-key kmap (kbd "C-s-@") #'rectangle-mark-mode)
    ;; s-@ not (yet) working, see C-x @ for event modifier adding
    (define-key kmap (kbd "C-s-SPC") #'pop-global-mark)
    (define-key kmap (kbd "s-a") (keymap-global-lookup "C-x a"))
    (define-key kmap (kbd "s-b") #'switch-to-buffer)
    (define-key kmap (kbd "C-s-b") #'bs-show)
    (define-key kmap (kbd "C-s-c") #'save-buffers-kill-terminal)
    (define-key kmap (kbd "s-d") #'dired)
    (define-key kmap (kbd "C-s-d") #'list-directory)
    (define-key kmap (kbd "s-e") #'kmacro-end-and-call-macro)
    (define-key kmap (kbd "C-s-e") #'eval-last-sexp)
    (define-key kmap (kbd "s-f") #'set-fill-column)
    (define-key kmap (kbd "C-s-f") #'find-file)
    (define-key kmap (kbd "s-g") #'magit-status)
    (define-key kmap (kbd "M-s-g") #'magit-dispatch)
    (define-key kmap (kbd "s-h") #'mark-whole-buffer)
    (define-key kmap (kbd "s-i") #'insert-file)
    (define-key kmap (kbd "C-s-i") #'indent-rigidly)
    (define-key kmap (kbd "C-s-j") #'dired-jump)
    (define-key kmap (kbd "s-k") #'kill-buffer)
    (define-key kmap (kbd "C-s-k") #'kmacro-keymap)
    (define-key kmap (kbd "C-s-k k") #'kmacro-end-or-call-macro)
    (define-key kmap (kbd "s-l") #'count-lines-page)
    (define-key kmap (kbd "C-s-l") #'downcase-region)
    (define-key kmap (kbd "C-s-m") (keymap-global-lookup "C-x C-m"))
    (define-key kmap (kbd "s-n") (keymap-global-lookup "C-x n"))
    (define-key kmap (kbd "C-s-n") #'set-goal-column)
    (define-key kmap (kbd "s-o") #'other-window)
    (define-key kmap (kbd "C-s-o") #'delete-blank-lines)
    (define-key kmap (kbd "s-p") (keymap-global-lookup "C-x p"))
    (define-key kmap (kbd "C-s-p") #'mark-page)
    (define-key kmap (kbd "s-q") #'kbd-macro-query)
    (define-key kmap (kbd "C-s-q") #'read-only-mode)  ; on mac this is lock screen, but this works with shift-q
    (define-key kmap (kbd "C-M-s-q") #'quit-restore-other-window)
    (define-key kmap (kbd "s-r") (keymap-global-lookup "C-x r"))
    (define-key kmap (kbd "C-s-r") #'find-file-read-only)
    (define-key kmap (kbd "s-s") #'save-some-buffers)
    (define-key kmap (kbd "C-s-s") #'save-buffer)
    (define-key kmap (kbd "s-t") (keymap-global-lookup "C-x t"))
    (define-key kmap (kbd "C-s-t") #'transpose-lines)
    ; (define-key kmap "s-u" #'undo)  ; here is revert-buffer
    (define-key kmap (kbd "s-u") #'revert-buffer)
    (define-key kmap (kbd "C-s-u") #'upcase-region)
    (define-key kmap (kbd "s-v") (keymap-global-lookup "C-x v"))
    (define-key kmap (kbd "C-s-v") #'navigate-window)
    (define-key kmap (kbd "s-w") (keymap-global-lookup "C-x w"))
    (define-key kmap (kbd "C-s-w") #'write-file)
    (define-key kmap (kbd "s-x") (keymap-global-lookup "C-x x"))
    (define-key kmap (kbd "C-s-x") (lambda (&optional arg) (interactive "P") (exchange-point-and-mark (not arg))))
    (define-key kmap (kbd "s-z") #'repeat)
    (define-key kmap (kbd "C-s-z") #'repeat)
    (define-key kmap (kbd "s-;") #'comment-set-column)
    (define-key kmap (kbd "C-s-;") #'comment-line)
    (define-key kmap (kbd "s-0") #'delete-window)
    (define-key kmap (kbd "s-1") #'delete-other-windows)
    (define-key kmap (kbd "s-2") #'split-window-below)
    (define-key kmap (kbd "s-3") #'split-window-right)
    (define-key kmap (kbd "s-4") #'ctl-x-4-prefix)
    (define-key kmap (kbd "C-M-s-SPC") #'jump-global-mark-other-window)
    (define-key kmap (kbd "s-5") #'ctl-x-5-prefix)
    (define-key kmap (kbd "s-6") #'2C-command)
    (define-key kmap (kbd "s-8 RET") #'insert-char)
    (define-key kmap (kbd "s-8 e") (keymap-global-lookup "C-x 8 e"))
    (define-key kmap (kbd "C-s-0") #'text-scale-adjust)
    (define-key kmap (kbd "C-s-+") #'text-scale-adjust)
    (define-key kmap (kbd "C-s--") #'text-scale-adjust)
    (define-key kmap (kbd "C-s-=") #'text-scale-adjust)
    (define-key kmap (kbd "C-M-s-0") #'global-text-scale-adjust)
    (define-key kmap (kbd "C-M-s--") #'global-text-scale-adjust)
    (define-key kmap (kbd "C-M-s-+") #'global-text-scale-adjust)
    (define-key kmap (kbd "C-M-s-=") #'global-text-scale-adjust)
    (define-key kmap (kbd "s-#") #'server-edit)
    (define-key kmap (kbd "s-$") #'set-selective-display)
    (define-key kmap (kbd "s-'") #'expand-abbrev)
    (define-key kmap (kbd "s-(") #'kmacro-start-macro)
    (define-key kmap (kbd "s-)") #'kmacro-end-macro)
    (define-key kmap (kbd "s-<") #'scroll-left)
    (define-key kmap (kbd "s-=") #'what-cursor-position)
    (define-key kmap (kbd "s->") #'scroll-right)
    (define-key kmap (kbd "s-[") #'backward-page)
    (define-key kmap (kbd "s-\\") #'activate-transient-input-method)
    (define-key kmap (kbd "s-]") #'forward-page)
    (define-key kmap (kbd "s-{") #'shrink-window-horizontally)
    (define-key kmap (kbd "s-}") #'enlarge-window-horizontally)
    (define-key kmap (kbd "s-*") #'calc-dispatch)
    (define-key kmap (kbd "s-+") #'balance-windows)
    ;; this is actually #'fit-window-to-buffer - see also #'tear-off-window
                                        ;(define-key kmap "s--" #'shrink-window-if-larger-than-buffer)
    (define-key kmap (kbd "s--") #'negative-argument)
    (define-key kmap (kbd "s-.") #'set-fill-prefix)
    (define-key kmap (kbd "s-^") #'enlarge-window)
    (define-key kmap (kbd "s-&") #'enlarge-window)
    (define-key kmap (kbd "s-`") #'next-error)
    (define-key kmap (kbd "s-<backspace>") #'backward-kill-sentence)
    (define-key kmap (kbd "s-<return>") #'execute-extended-command)
    (my-env-add-map kmap)))
#+end_src

*** handling of mark, region and transient mark mode

Using marks not only as region delimiters, but also as storage of previous
locations is an intriguing idea. Supporting this, we use the /super/ key
bindings introduced above to implement a variation of ~exchange-point-and-mark~,
which reverses the effect of prefix arg. Now we can use =C-(x x)= as usual
exchanging point and mark and activating the transient region; and =C-s-x=
just exchanging point and mark without activating the region. This way it
feels really simple to jump back and forth between current and previous
location. Otherwise use =C-(u SPC)= repeatedly to iterate through the ring.
In addition, using ~visible-mark-mode~ being able to see the mark makes this
really convenient.
...
Combined with ~delete-selection-mode~, this works nicely and as expected: when region is
deactivated, we can just type ahead to write text. If the mark is on the same line
as point, it moves on while typing. When region is activated, instead, it is
replaced by the text typed, which is just what I want.

In addition, the change below in [[*delete-selection minor mode][delete-selection minor mode]] makes sure the
replaced region is also put into kill ring, making it very convenient
to prepend or surround a region: just type the prepending text, then =C-y= and
the final enclosing text.

The final adaptation, also pushing the replaced region of a yank to kill ring,
makes it very convenient (and conforming to expectations) to just use =C-(y
y)=, this way first replacing the region with the head of the kill ring, then
inserting the new head (the text just replaced). Of course, you can achieve
exactly this by deactivating the region and then inserting the yank. Also,
when assigning a register to ~delete-selection-save-to-register~, the
text replaced by a yank is saved to that register. Now just re-insert it with
=s-r i y= (for instance, after assigning the =y= register). And again,
~insert-register~ works as expected when applied to an active region by just
pushing replaced regions to the kill ring.

But see also [[https://spwhitton.name/blog/entry/transient-mark-mode/][GNU Emacs' Transient Mark mode]] regarding the question of
(de)activating transient mark mode.

**** navigating mark-ring more conveniently

Using =C-(u SPC)= always feels somewhat off. As it shows, the reason is that
while pushing current mark to the tail of the mark-ring, setting point to it's
position and popping a new mark off the mark-ring, the initial position of
point is lost. So you can rotate through all positions of mark and mark-ring,
but you won't ever reach your starting point.
Instead, here is a function rotating through point, mark and mark-ring. That
is, if mark-ring is empty it does just the same as ~exchange-point-and-mark~.

#+begin_src emacs-lisp
(defun my/mark-navigate-backward ()
  "save point to tail of mark-ring, then set point to mark and pop new mark off head of mark-ring."
  (interactive)
  (if (null (mark t))
...
And here is a function rotating forward, in the other direction.

#+begin_src emacs-lisp
(defun my/mark-navigate-forward ()
  "push mark to head of mark-ring, set mark to point and point to tail of mark-ring."
  (interactive)
  (if (> (length mark-ring) 0)
      (let ((lastpos (marker-position (nth (1- (length mark-ring)) mark-ring)))
            (mark-ring-max (length mark-ring)))
        (push-mark lastpos t nil)))
  (exchange-point-and-mark t))
#+end_src

Now, we can combine both into a transient function, enabling convenient
navigation through all local mark positions. In addition, if
~visible-mark-mode~ is activated, it displays all positions of mark-ring,
giving a nice visual feedback.

#+begin_src emacs-lisp :lexical t
(defun my/set-visible-marks (num)
  (interactive "p")
  (visible-mark-mode 0)
  (setq visible-mark-max num)
  (visible-mark-mode 1))

(defun my/navigate-mark-ring ()
  "using visible-mark-mode, display all marks and navigate back or forward by means of a
   repeated transient keymap. Depends on visible-mark-mode being activated."
  (interactive)
  (let ((visible-marks visible-mark-max))
    (my/set-visible-marks mark-ring-max)
    (compat-call set-transient-map
     (let ((map (make-sparse-keymap)))
       (define-key map "n" #'my/mark-navigate-forward)
       (define-key map "p" #'my/mark-navigate-backward)
       map)
     t
     (lambda ()
       (my/set-visible-marks visible-marks))
     "navigate mark-ring (%k)")))

(global-set-key (kbd "M-g m") #'my/navigate-mark-ring)
#+end_src

And finally a function activating the last region:

#+begin_src emacs-lisp
(defun my/activate-last-region ()
  (interactive)
  (my/mark-navigate-backward)
  (activate-mark))
#+end_src

cleaning the mark-ring:
#+begin_src emacs-lisp
;; (dolist (ml mark-ring) (set-marker ml nil))
;; (setq mark-ring nil)
...
** Remote controlling of other window
...
#+BEGIN_SRC emacs-lisp
(global-set-key "\221" 'quit-restore-other-window)  ;; C-M-q
(global-set-key [201326652] #'beginning-of-other-window)  ;; C-M-<
(global-set-key [201326654] #'end-of-other-window)  ;; C-M->
...
(global-set-key (kbd "C-M-s-s") #'isearch-forward-other-window)
...
(global-set-key [142606358] #'my-scroll-other-window-down)  ;; C-M-s-v
#+END_SRC
...
** Remote controlling of other frame
...
#+BEGIN_SRC emacs-lisp
(setenv "PATH" (concat "/usr/pkg/bin:/opt/local/bin:/opt/local/sbin:" (getenv "PATH")))
...
;; (require 'ox-rst) ; not yet installed
(require 'yaml-mode)
...
(load "lilypond-init.el")  ;; LilyPond-mode
...
;; (autoload 'c++-mode   "cc-mode" "C++ Editing Mode" t)  ;; needed?
...
** delete-selection minor mode
This mode, when activated, deletes the active region (and probably also saves
it into a register) if some kind of text insertion or modification command is
issued. As a side note, when deactivated, you can just type without deleting
the region.

But instead, I'd expect this kind of actions to kill (that is, put it into the
kill ring) the active region. After all, this should change even small single
character delete actions into larger region-changing kill actions.

So here is an experimental advice function, augmenting ~#'delete-selection-helper~ and
changing the action type to ~'kill~ for certain types.

Now this way, you can use the pattern of selecting text, then typing on
(replacing it) and finally using =C-y= to re-insert the original text (like
select, then type =" C-y "=).

#+begin_src emacs-lisp
(setq my/selection-type nil)

(defun my/selection-advice (oldfun type)
         (let ((type
                (if (or (null type)
                        (functionp type)
                        (eq type 'yank)
                    ...
                    type
                  'kill)))
           (setq my/selection-type type)
           (funcall oldfun type)))

(advice-add 'delete-selection-helper :around #'my/selection-advice)
(setq delete-selection-save-to-register ?y)
#+end_src

In case of problems while testing this modification, use =(advice-remove 'delete-selection-helper #'my/selection-advice)=
to deactivate it.

Just for sake of consistency, a yank is handled the same way. That is, yanking
first saves the replaced region into a temporary register, then yanks in the
top of the kill ring, and finally puts the temporary register (the previously
saved original region) on top of the kill ring.

#+begin_src emacs-lisp
(defun my/selection-post ()
  (if (and (eq my/selection-type 'yank)
           delete-selection-save-to-register)
      (progn
        (setq my/selection-type nil)
        (kill-new (get-register delete-selection-save-to-register)))))

(add-hook 'post-command-hook #'my/selection-post)
#+end_src

In case of problems while testing this modification, use =(remove-hook 'post-command-hook #'my/selection-post)=
to deactivate it.

** dired
*** some general remarks
dired is more than just a simple replacement for ~ls~. 
When opening two dired buffers, the second one is used as default taget for
copy/move operations from files of the first one.

Files can be marked interactively or by different kinds of types (see =*=). Marked image
files can be displayed as thumbs within the dired buffer (=C-t C-t=).
Commands or pipelines can be applied to all marked files. That's like an
interactive kind of =xargs=. Use =!= for sync, =&= for async commands. This
command is applied for each marked file, with its name appended. Otherwise,
use =&= as placeholder to insert name at arbitrary places within the command,
or use =*= to insert list of all file names into a single call.
If no files are marked, the file under point is target. Prefix arg additionally selects the
next n files after the file under point.

*** some handy keys
 =C-x C-j= :: open dired of current dir

 =s-4 C-j= :: open dired of current dir in other window

 =C-o= :: open file under point without visiting it

 =v= :: open file under point in view-mode (=q= returns)

 =q= ::  close dired buffer

 =(= ::  toggle visibility of details

 =i= ::  insert directory into view

 =k= ::  hide files. =C-u k=: remove inserted directories

 =* %= :: mark files selected by regexp

 === ::  compare files at mark and point 

 =m= ::  mark files in region (or under point)
...
(define-key dired-mode-map ";" #'dired-up-directory)
(define-key dired-mode-map "ˆ" #'dired-up-directory)  ;; on german keyboard same key like ;
...
(define-key dired-mode-map "="
  (lambda ()
    ...
    (let ((file (dired-get-filename))
          (prev (save-excursion (goto-char (mark t))(dired-get-filename))))
      (if (and file prev)
...
General idea is to use all text filter commands in the usual way: a text
filter (optionally) takes in some text from stdin, does something and outputs
the result to stdout. On the opposite, visual commands (or better interactive
commands) are modifying the terminal somehow (like tmux, top, vi, more). As
eshell uses comint-mode (instead of term-mode), they don't work here directly.
So they are run inside of another buffer being in term-mode. The list of
visual commands is customized in ~eshell-visual-commands~,
~eshell-visual-options~ and ~eshell-visual-subcommands~. Setting
~eshell-destroy-buffer-when-process-dies~ to ~nil~ results in the same
behaviour as usual: when quitting these commands, we are returned directly to
eshell, which works nicely.

In a more emacs-like way, the idea is to not use terminal, but instead replace
most of these commands by native functions (like vi - evil, man -
manual-entry, top - proced). The function of tmux is already inherently
present in emacs.
The respective  replacement should open a buffer in current eshell window and generally on =q=
return to eshell, acting similarly to having the alternate screen in
terminals.
Tools that would paginate, like git log, should be configured to just output away,
as smart scrolling handles that case quite nicely.

*** howtos

- Buffers can be addressed like file redirection: #<buffer name>. This is the
  same as ~(get-buffer-create "name")~.
  A third redirection function ~>>>~ appends to current point if target is a
  buffer.

- Use lisp functions as shell commands: ~switch-to-buffer-other-window
  #<buffer>~.

- See ~cd~ history: ~cd =~. Address old dirs: ~$-[n]~ or ~$OLDPWD[n]~.

- Start a command sending output into extra buffer: ~compile <command>~.

- ~occur~: search in current eshell buffer.

- There are virtual devices:
  ~/dev/eshell~ :: output into eshell window

  ~/dev/clip~   :: output to clipboard

  ~/dev/kill~   :: output into kill ring

- copy some selected previous output to command line: =C-0 C-c RET=
  directly execute it: =C-0 RET=
  (to copy the respective lines, don't use the prefix command).

This works also with other-window-prefix:
#+begin_src emacs-lisp
...
This can be used to output the contents of a buffer: ~buffer "test" | cat~
Narrow it (=C-x n=) to output only the narrowed part.
If the lines contain shell commands, each of them can be executed with ~buffer "test" | sh~.
#+begin_src emacs-lisp
(defun eshell/buffer (name)
  (with-current-buffer name (buffer-string)))
#+end_src

A shortcut for ~dired~.
#+begin_src emacs-lisp
...
(defun eshell/la (&rest args)
  (apply 'eshell/ls "-al" args))

(defun eshell/vi (name)
  (find-file name)
  (evil-normal-state)
  (recursive-edit)
  (evil-emacs-state)
...
(defun eshell/moccur (&rest args)
  (eshell-poor-mans-grep args))
#+end_src

Reload our defined aliases:
#+begin_src emacs-lisp
;;(eshell-read-aliases-list)
#+end_src

Enable copying of region with =C-u C-c RET=:
#+begin_src emacs-lisp
(defun eshell-get-region-or-input (&optional use-current-region)
...
(defun eshell-copy-region (&optional arg)
  "copy line or region to prompt, without executing it.
With prefix, use region."
  (interactive "P")
  (let ((input (eshell-get-region-or-input arg)))
    (deactivate-mark)
    (goto-char eshell-last-output-end)
    (insert-and-inherit input)))

(define-key eshell-mode-map (kbd "C-c RET") #'eshell-copy-region)
#+end_src

*** paging/viewing in eshell

We want to have an explicit way to paginate output, like ~more~ in traditional
shells. There seems to be no easy way (currently) to implement eshell commands
handling input and output in lisp. Instead we implement a virtual device,
which is registered in ~eshell-virtual-targets~.

Output goes to a buffer called ~*viewer*~ in ~view-mode~. When overwriting, it is erased,
otherwise we append to the end. The buffer is made visible and finally using
~q~ in this mode buries it and returns to eshell.

#+begin_src emacs-lisp
(defun my-eshell-to-view-buffer (mode)
  (let ((my-buffer (get-buffer-create "*viewer*")))
    ...
    (lambda (string)
      (with-current-buffer my-buffer
        ...
        (save-excursion
          (end-of-buffer)
          (if (char-or-string-p string)
              (insert string)))
        (view-mode 1)
        (switch-to-buffer my-buffer)))))
#+end_src

Now, an alias of more 'cat $1 > /dev/view' does the trick. We can call ~more
file~ or pipe to it: ~ls|more~. Both ways it works as expected.

*** todo
 - find out about the pecularities of smart scrolling
 - output like git branch/infos on dir change:
   see eshell customizing options
    see eshell-directory-change-hook
    see eshell-prompt-function
    see epe (eshell prompt extras)
   
** Evil mode

*** Preparations

We want to enter evil only occasionally; we use it with the undo-tree package.
...
Also, we never want evil to quit emacs.
#+begin_src emacs-lisp
(setf (symbol-function 'evil-quit) #'exit-recursive-edit)
#+end_src
...
(defun evil-rc ()
  ...
  (evil-define-key 'normal 'global (kbd "<leader>p") #'evil-prev-buffer)
  (evil-define-key 'normal 'global (kbd "<leader>l") #'global-display-line-numbers-mode)
  (evil-define-key 'normal 'global (kbd "<leader>x") (key-binding "\C-x"))
  (evil-define-key 'normal 'global (kbd "<leader>c") (key-binding "\C-c"))
...
(defun def-evil-scroll-state ()
  ...
  (evil-define-key 'normal 'global (kbd "<leader>s") 'evil-scroll-state))
#+END_SRC
...
Lets try to put a hide-lines prefix on =C-c h=:
(side note, never put C-h into a key, as this shadows the help facility)
#+begin_src emacs-lisp
(autoload 'hide-lines "hide-lines"
  "Hide lines based on a regexp" t)
(require 'hide-lines)
(define-prefix-command 'my-hide-lines-map)
(global-set-key (kbd "C-c h") my-hide-lines-map)
(define-key my-hide-lines-map (kbd "r") #'hide-lines)
(define-key my-hide-lines-map (kbd "s") #'hide-lines-show-all)
#+end_src
Note with prefix, its function is inversed. With two prefixes (=C-u C-u=) all
hidden lines are shown again.

Now this works with regexes, which is cool. But more emacs-like would be to
work on regions and lines.

#+begin_src emacs-lisp
(defun hide-lines-selection (&optional prefix)
  "Hide lines. If a region is selected, hide that. Otherwise, hide the current
line. With a numeric prefix argument, hide that number of lines."
  (interactive "p")
  (if (region-active-p)
      (progn
        (hide-lines-add-overlay (mark) (point))
        (deactivate-mark))
      (beginning-of-line)
      (let ((start (point)))
        (forward-line prefix)
        (hide-lines-add-overlay start (point)))))
#+end_src

#+begin_src emacs-lisp
(define-key my-hide-lines-map (kbd "h") #'hide-lines-selection)
#+end_src

And here a small helper, extracting the current, possibly non-contiguous region:
#+begin_src emacs-lisp
(defun region-string ()
  (cl-labels
       ((nl-string (str)
...
*** invert visible lines

Using the advice function of next section, it is quite easy to invert the
whole buffer's visible lines. Marking the whole buffer will give just that
non-contiguous list of ~region-bounds~ which we will use to hide each of them.

#+begin_src emacs-lisp
(defun my/hide-lines-inverse ()
  "invert hidden lines. That is, hide all visible lines, show all others."
  (interactive)
  (save-excursion
    (mark-whole-buffer)
    (let ((visible-lines (region-bounds)))
      (hide-lines-show-all)
      (mapcar
       (lambda (region)
         (hide-lines-add-overlay (car region) (cdr region)))
       visible-lines)))
  (deactivate-mark))
#+end_src

#+begin_src emacs-lisp
(define-key my-hide-lines-map (kbd "i") #'my/hide-lines-inverse)
#+end_src

*** non-contiguous region on hide-lines

Here we build an advice function for region-extract-function, setting a
non-contiguous region containing selected visible lines.
This way, we finally can hide some lines and then copy (or do whatever with)
the rest (like pipe it to a shell).

The first function transforms overlays of invisible areas into region
descriptions. Then we have a function reversing them, effectively describing
all remaining visible areas within the selected region.
Finally, an advice function for ~region-extract-function~, using these
noncontiguous bounds.

#+begin_src emacs-lisp
(defun my/region-from-overlay (overlay)
  (let ((beg (overlay-start overlay))
        (end (overlay-end overlay)))
    `(,beg . ,end)))

(defun my/visible-regions-helper (overlays beg end regions)
  (if (null overlays)
      (remove-if
       (lambda (elem)
         (>= (car elem) (cdr elem)))
       (append regions `((,beg . ,end))))
    (let ((invs (caar overlays))
          (inve (cdar overlays)))
      (let ((start (if (> invs beg) beg
                     (min (max inve beg) end)))
            (stop (if (< inve end) end
                    (max (min invs end) beg))))
        (let ((inv (min invs stop))
              (next (max inve start)))
          (my/visible-regions-helper
           (cdr overlays)
           next
           stop
           (append regions `((,start . ,inv)))))))))

(defun my/visible-regions (prev-regions)
  (let ((inv-regions (sort
                      (mapcar #'my/region-from-overlay
    ...
    (apply #'append
           (mapcar
            (lambda (region)
              (my/visible-regions-helper
               inv-regions
               (car region) (cdr region) nil))
            prev-regions))))

(defun my/region-bounds-advice (orig method)
  (cl-labels ((nl-string (str)
                (if (string-equal (string-chop-newline str) str)
                    (format "%s\n" str)
                  str))
              (del-region (bounds)
                (mapcar
                 (lambda (region) (delete-region (car region) (cdr region)))
                 bounds))
              (get-region (bounds)
                (apply #'concat
                       (mapcar
                        (lambda (region)
                          (nl-string (filter-buffer-substring (car region) (cdr region) nil)))
                        bounds))))
    (if (or
         (not (region-active-p))
         (not hide-lines-invisible-areas))
        (funcall orig method)
      (let ((my-bounds (my/visible-regions (funcall orig 'bounds))))
        (cond
         ((eq method 'bounds)
          my-bounds)
         ((eq method 'delete-only)
          (del-region (reverse my-bounds)))
         ((eq method 'delete)
          (let ((region (get-region my-bounds)))
            (del-region (reverse my-bounds))
            region))
         (t
          (get-region my-bounds)))))))

(add-function :around region-extract-function #'my/region-bounds-advice)
#+end_src

*** appending to buffers from non-contiguous regions

#+begin_src emacs-lisp
(defun my/append-to-buffer (buffer)
  (interactive
   (list
    (read-buffer "Append to buffer: " (other-buffer (current-buffer) t))))
  (let ((regions (region-bounds)))
    (mapc (lambda (region)
            (append-to-buffer buffer (car region) (cdr region)))
          regions)))
#+end_src

This works in the same way as the original append-to-buffer, as in
rectangle-mode, the selected line snippets are appended without line breaks.
But this also happens if you just mark parts of a line and append them without
editing the target buffer. So ~append-to-buffer~ works in a very rudimentary
way, as long as you don't select full lines.

This could be mitigated by advising to the original function, using
~(region-string)~ if ~region-noncontiguous-p~, otherwise using the original
function.

But many other functions like ~occur~ also only work with simple contiguous
regions.
...
We want =nameless-mode= to support ":" and "/" as namespace separators.

#+begin_src emacs-lisp
(setq nameless-private-separator "/")

(defun nameless--name-regexp (name)
  "Return a regexp of the current name, supporting private separator."
  (concat "\\_<@?\\(" (regexp-quote name)
          "[" (or nameless-separator "") (or nameless-private-separator "")  "]\\)\\(\\s_\\|\\sw\\)"))
#+end_src
** org
The well known template mechanism =<s TAB= has been replaced by a
mechanism based on the tempo package (but see also below). This is activated by

#+begin_src emacs-lisp
(require 'org-tempo)
#+end_src

*** org structures, templates and litterate programming
=C-c C-,= can be used to insert structure templates,
which also works with regions.
Use =M-TAB= for completion. E.g. when inserting a src block, use this to
complete language and options.
=C-c '= within a structure opens its contents in a special buffer using the
corresponding mode to edit. This way, write code or evaluate lisp forms using
=C-M-x=. When writing interactive notebooks, use =C-c C-c= to execure code
within a structure. The output will then be included in a result block.

*** org outline editing, markup, lists, tables

Use left, right, up, down with =M= to move nodes around (how to do this on
keyboards without cursor keys?).
Use =M-RET= to insert a new node at point, with same level as previous one.
Use =C-RET= to insert a new node below point, after all subsequent nodes and
before the next one of same level.
=C-c= prefix together with =C-n=, =p=, =f=, =b=, =u= navigate through the
document. =C-c C-j= shows document structure and allows jumping.
Use =C-c C-x C-f= as helper for marking regions.
Lists can be unordered (-, +, *), ordered: 1. or 1) or use descriptions
(separating term :: description).
...
indenting in python buffers.

*** (Re-)Installing venv

First, make sure to set custom variables like ~elpy-rpc-python-command~ to
appropriate values. When resetting them, probably also remove the virtual env
directory in ~.emacs.d/elpy/rpc-venv~. Afterwards, use
~elpy-rpc-reinstall-virtualenv~ to regenerate it. This is used e.g. for
finding documentation and object definitions. See also ~elpy-rpc-restart~ and
~jedi:install-server~ etc.

*** Using elpy

Use =C-c C-d= for documentation for item under
...
Also see the elpy refactoring methods, like =C-c C-e= for multi-editing symbol
under point (change name and it is simultaneously changed in all occurences).
...
(define-key python-mode-map (kbd "<M-tab>") #'elpy-folding-toggle-at-point)
#+END_SRC

Enable autopep8, but don't activate it on save. 
Also don't use =M-g M-n= and =M-g M-p= for flycheck next and flycheck previous
error, because we want to use this to navigate through occurence buffers.
Instead, remap =M-g n= and =M-g p= to flycheck next and previous error, which
also show pep8 errors. The flycheck menu can also be found with =C-c !=.

#+BEGIN_SRC emacs-lisp
(require 'py-autopep8)
(add-hook 'elpy-mode-hook 'py-autopep8-enable-on-save)
(define-key elpy-mode-map "\M-gp" #'flycheck-previous-error)
(define-key elpy-mode-map "\M-gn" #'flycheck-next-error)
#+END_SRC
...
Using ~customize~, tramp can be configured (via ~tramp-connection-properties~)
to export ssh-agent to certain target hosts. This comes in handy when using
magit on remote connections.

When using the tramp ssh method, opening a term should re-use the ssh control.
This code should check if a controlled connection is actually established
before trying to use it. On the other side, this can be accessed only from
within a file opened via tramp. At least, the host name check could re-use the
~tramp-connection-properties~ mechanism.
...
The term buffer is named based on `name' and behaves like `ansi-term'."
  (interactive)
  (require 'term)
  (require 'tramp)
  (let ((path (replace-regexp-in-string "^file:" "" (or path default-directory)))
        (name (or name "ansi-term"))
        (cd-str "fn=%s; if test ! -d $fn; then fn=$(dirname $fn); fi; cd $fn; exec bash"))
    (cl-labels ((start-term (termbuf)
                  (set-buffer termbuf)
                  (term-mode)
                  (term-char-mode)
      ...
      (if (tramp-tramp-file-p path)
          (let* ((tstruct (tramp-dissect-file-name path))
                 ...
                 (switches (list (if (equal (tramp-file-name-host tstruct) "con-jko-linux.local")
                                     "-A" nil)
                                 "-q"
...
* Best practices
** customizing compilation buffers
- M-x customize-option compilation-error-regexp-alist
- add error spec: ("\\[ERROR\\]\\W*\\[\\([^:]*\\):\\(.*\\)\\]" my-fqpn 2)
  so the regex string needs every "\" to be quoted
  - convenience: use helm-regex, test it within log buffer
  - when finished, RET quotes and copies it to kill ring
- here match group 1 is file, group 2 is lineno
  file is modified by a function, receiving no arg, so it fetches last match subgroup 1
  (defun my-fqpn ()
    (concat (replace-regexp-in-string "\\." "/" (match-string-no-properties 1)) ".py"))
- open logfile
- M-x compilation-minor-mode
- convenience: (setq default-directory "/path/to/project_root")
- convenience: use hide-lines to filter down log
  C-u C-c C-h r \[cs.catalyst
- convenience: M-x next-error-follow-minor-mode

** Modifying regions
- use keyboard macro on region: =C-x C-k r=
- use rectangular region:
  - =C-x SPC= or =C-M-Mouse1=
  - interactively replace each line with text: =C-x r t=
    use this with rectangle of width 0, inserting text into each line at given column
- use shell command on region: =M-|=
  each line of region is input (stdin) of single instance of given shell
  command. Output goes to command buffer (or, with =C-u= prefix, replaces region)
  example:

  awk '{print $1}' :: replace each line by first word of line
  
  sh :: execute each line
** Replacements
- use quoted characters to insert control characters into searches or
  replacements:
  =M-%= query replace SPC =C-q C-j= :: replace space by RET
** Use diff-mode on diff-buffers
- for example, in eshell: emgit diff file|more
- M-x diff-mode
- C-x C-q (read-only)
  - M-n / M-p to step through diffs
  - C-c C-n to narrow to current diff
  - C-c C-r to reverse diff direction
** Send CTRL-chars to attached process
- for instance, when docker attach called in shell-mode or eshell-mode
  - M-x term-char-mode
  - afterwards the detach command seq C-p C-q works

** Skeletons

Skeletons are macros inserting text at point, either as is or with different
means of requesting text to be filled in. They can use the minibuffer to input
text (using initial query or calling ~skeleton-read~). Existing text can be
wrapped. Or, after inserting the template, the point can be moved into
positions to be filled afterwards.

Skeleton expansions behave differently mid-line or at the beginning or end of
a line. At the end of a line (or at an empty line), the insertion is just done
without adding newlines. At the beginning or middle of a non-empty line, the
rest is moved down before inserting. Here we handle this by actively preparing
an empty line before inserting. Quoted commands are executed just for
side-effect while the return value of unquoted functions will be inserted into
the expansion.

A skeleton inserting text:
#+begin_src emacs-lisp
(define-skeleton
  tmpl/org-name
  "insert org name"
...
Repeating subskeletons. The variable =str= is replaced by the result of an
interactor given in first position of the (sub-)skeleton. If the interactor is
a string, it is used as argument to ~skeleton-read~.
As long as the query for a header arg is answered with
a non-empty string, the sub-skeleton is repeated. Also, the =-= marks the
position the point is moved to after finishing. Finally, each =@= marks
additional positions. Using ~my/navigate-skeleton~, they can be reached
afterwards.
#+begin_src emacs-lisp
(define-skeleton
  tmpl/org-header
...
Simple example for wrapping existing text. Here, one region is expected
(marked =_=). The indentation command =>= indents literal or wrapped text.
#+begin_src emacs-lisp
(define-skeleton
  tmpl/indent
...
More than one placeholder of wrapped text can be given. A couple of previous
marks can then be used by giving a negative prefix arg (if a region is active
and no prefix arg given, then -1 is assumed, using the region between point
and the single mark set as last). A positive arg wraps the N next words.
If no prefix arg and region is given, the point is placed at the first =_=
afterwards. When =_= and =-= are given, then the last =-= overrides for
placing point.

This is also an example for conditional insertion. If fewer than three regions
are given, only the first replacement markers are wrapped. For each
replacement not given, a newline is inserted instead, the point being moved to
the first one not given.
#+begin_src emacs-lisp
(define-skeleton
  tmpl/if
  "conditional lisp execution"
  nil
  > "(if " _ | \n
  > @ _ | \n
  > @ _ ")")
...
Active queries can be used if no replacements are given. The query is only
executed when ~str~ is inserted.
#+begin_src emacs-lisp
(define-skeleton
  tmpl/if2
  "conditional lisp execution"
  "condition: "
  > "(if " _ | str
  > @ _ | (skeleton-read "expression if true: ")
  > @ _ ")")
#+end_src

Elements can be grouped in sub-skeletons having a nil interactor in first
position and not accessing =str=. The conditional elements =&= and =|= execute
the next element if the previous one did move respectively did not move point.
By grouping elements, you can have the last one as non-moving element (like
=@= or =-=) and still have a following =&= executed if former elements of the
group did move point. In the same way, you can group elements following a =&=
or =|=.

#+begin_src emacs-lisp
(define-skeleton
  tmpl/if3
  "Wrap form into conditional lisp expression.
Ask for test condition. If none is given, place point to insert condition afterwards.
If two regions are given, wrap them as true and false expressions.
If one region is given, wrap it as true expression and set point to insert false condition,
if condition is given. Otherwise, point is placed at condition.
If no region is given, also ask for true expression.
"
  nil
  > "(if " (nil (skeleton-read "condition: ") | -) & @ \n
  > _ | (skeleton-read "expression if true: ")
  > @ _ & (nil ")" '(my/pop-skel-pos)) | ")")
#+end_src
  
Best practice is using just one =_ | str=, wrapping region if given and
querying if not.

A simple in-line expansion without line breaking:
#+begin_src emacs-lisp
(define-skeleton
  tmpl/call_inline
  "insert an inline org babel block call."
  "block: "
  '(cycle-spacing)
  "call_" _ | str "("-")"
  '(cycle-spacing)
...
Dynamically execute subskeletons as long as inter-regions can be consumed. The
query is a function or form instead of a string. This form must evaluate to a
string for the subskeleton to be expanded or =nil= to finish. Also, =str= must
be addressed to execute the query. Here, that's done just in form of a
side-effekt. The query form must be guaranteed to terminate, that is, at some
point evaluate to =nil=. Here it is achieved by consuming each skeleton-region
using =_=.
#+begin_src emacs-lisp
(define-skeleton
  tmpl/skel-recursive
  "put each region into a block."
  nil
  -
  (
   (let ((v1 (first skeleton-regions)))
     (if (markerp v1)
         (buffer-substring-no-properties (point) (marker-position v1))
       nil))
   '(skeleton-read str)
   "#+BEGIN_example\n"
   @ _
...
*** Navigating skeletons

Just like navigate-mark-ring:
#+begin_src emacs-lisp :lexical t
(defun my/pop-skel-pos ()
  (interactive)
  (setq skeleton-positions (nconc skeleton-positions (list (point))))
  (goto-char (pop skeleton-positions)))

(defun my/push-skel-pos ()
  (interactive)
  (push (point) skeleton-positions)
  (goto-char (first (last skeleton-positions)))
  (setq skeleton-positions (nbutlast skeleton-positions)))

(defun my/navigate-skeleton ()
  "Given marked positions of an inserted skeleton, navigate back or forward
by means of a repeated transient keymap."
  (interactive)
  (compat-call set-transient-map
   (let ((map (make-sparse-keymap)))
     (define-key map "n" #'my/push-skel-pos)
     (define-key map "p" #'my/pop-skel-pos)
     map)
   t
   nil
   "navigate skeleton (%k)"))

(global-set-key (kbd "M-g s") #'my/navigate-skeleton)
#+end_src

** example for automating org block content modification

The hook needs to find it's surrounding. ~narrow-to-block~ not always is a
good idea as it also narrows to the previous block even if point is behind it.

#+begin_src emacs-lisp
(defun my-org-example-test ()
  (save-mark-and-excursion
    (org-narrow-to-block)
    (goto-char 0)
    (let ((name (buffer-substring-no-properties (point) (+ 15 (point)))))
      (if (equal (upcase name) "#+BEGIN_EXAMPLE")
          (progn
            (mark-whole-buffer)
            (replace-regexp " " "\n")
            (deactivate-mark)
            ...
            (next-line)
            (beginning-of-line)
            (while (not (equal (upcase (buffer-substring (point) (+ 5 (point)))) "#+END"))
              (insert "  - ")
              (next-line)
              (beginning-of-line))
            (widen)
...
;; (push #'my-org-example-test org-ctrl-c-ctrl-c-hook)
#+end_src

** recursive structured editing using multiline regexps

Each match of ~query-replace-regexp~ can be used as region, using that while
going into a deeper level of recursive editing. There, for instance, =C-x n n=
can conveniently narrow to region.
#+begin_src emacs-lisp
(defun my/edit-match-region ()
  (save-mark-and-excursion
...
Multiline regexps can be built like this:
"^(defun\\(.*\n\\)*?\n"
When entering them interactively, use =C-q C-j= instead of "\n".
Also see ~(helm-regexp)~ and ~(re-builder)~ supporting regexp building.
